...
class SSP {
	...
	static function filter ( $request, $columns, $filtroAdd )
	{
		...
		if ( isset($request['search']) && $request['search']['value'] != '' ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) {
					$globalSearch[] = " {$column['tb']}."."{$column['db']}::varchar ILIKE '%$str%'";
				}
			}
		}
		...
		for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
			...
			if ( $requestColumn['searchable'] == 'true' &&
			 $str != '' ) {
				$columnSearch[] = " {$column['tb']}."."{$column['db']}::varchar ILIKE '%$str%'";
			}
		}
	...
	static function simple ( $request, $pg_details, $table, $primaryKey, $columns, $filtroAdd=NULL )
	{
		...
		$select = "SELECT ".implode(", ", SSP::pluckas($columns,'tb','db','as'))."
