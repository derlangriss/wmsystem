...
if ($action == 'INSERT' || $action == 'UPDATE') {
    ...
    $texpense_cost          = $obj->expense_cost;
    $idproject_budgettype  = $obj->idproject_budgettype;
    $idbudget_type  = (int)$obj->idbudget_type;
...
if ($action == 'DELETE' || $action == 'DELETEALL') {
    ...
    if ($action == 'DELETE') {
        $query2 = "WITH update_expect AS ( ";
        $query2 .= "UPDATE project_details_expect ";
        $query2 .= "SET expect_cost = 0 ";
        $query2 .= "WHERE project_details_idproject_details = " . $idproject_details . " ";
        $query2 .= ") ";
        $query2 .= "UPDATE project_details ";
        $query2 .= "SET expense_cost = 0 ";
