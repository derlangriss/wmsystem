...
if (isset($_GET['idproject_details'])&&isset($_GET['idproject_budget'])) {

    $query = "WITH budgetcost AS (
        SELECT budget FROM project_budget
        WHERE idproject_budget = " . $_GET['idproject_budget'] . "
     ), sumexpensecost AS (
        select sum(expense_cost) includeexpect from project_details
        ...
        SELECT expense_cost,expense_type FROM project_details
        ...
        SELECT (SELECT budget FROM budgetcost)-(SELECT includeexpect FROM sumexpensecost) AS balancebudget,
        (SELECT expense_type FROM expensecost),
        (SELECT expense_cost FROM expensecost),
        (SELECT includeexpect FROM sumexpensecost)";
